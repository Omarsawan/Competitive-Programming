...
public class liChaoTreeMax{
	treeNode root;
	double minN,maxN;
	liChaoTreeMax(double min,double max){
	...
	double get(double x, treeNode node, double left, double right) {
	    ...
	    if(x <= mid) {
	        return Math.max(valueAt(node.val, x), get(x, node.left, left, mid));
	    }
	    return Math.max(valueAt(node.val, x), get(x, node.right, mid, right));
